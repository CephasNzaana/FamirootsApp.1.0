...
const transformTreeFormDataToMembers = (
    ...
    const getFullClanElder = (elderId: string, tribeNameHint?: string, clanNameHint?: string): FullClanElderType | undefined => {
        for (const tribe of ugandaTribesData) {
            ...
            for (const clan of tribe.clans) {
                if (clanNameHint && clan.name !== clanNameHint) continue; // Check if clanNameHint is provided
                const found = clan.elders?.find(e => e.id === elderId);
                if (found) return { ...found, clanName: clan.name, clanId: clan.id, // Add context
                                    // Ensure parentId from data is used, not one from a previous transform
    ...
    const addHistoricalLineageBranch = (
        ...
        lineageElderTribeName: string | undefined,
        lineageElderClanName: string | undefined,
        attachmentPointFmId: string | undefined, // Can be undefined if linking to main person directly
        attachmentPointGen: number,
        lineageSide: 'paternal' | 'maternal' | undefined
    ) => {
        if (!lineageElderRef?.id || !attachmentPointFmId) return; // Ensure attachment point exists

        const primaryLineageElder = getFullClanElder(lineageElderRef.id, lineageElderTribeName, lineageElderClanName);
        ...
        // Trace up to 2 ancestors + Tribal Ancestor (max 3 historical figures in the chain including PLE)
        for (let i = 0; i < 3 && currentElderToTrace; i++) {
            ...
            if (currentElderToTrace.parentId) {
                if (currentElderToTrace.parentId.startsWith("TA_")) {
                    ...
                    const tribeForTA = ugandaTribesData.find(t => t.id === taId.substring(3)) || ugandaTribesData.find(t => t.name === lineageElderTribeName);
                    const taName = tribeForTA ? `Progenitor of ${tribeForTA.name}` : "Tribal Progenitor";
                    
                    // Add TA node using its original ID if not already added
                    const taFmId = addPerson(null, { name: taName, gender: 'male' }, "Tribal Progenitor", attachmentPointGen - historicalChain.length -1 , true, undefined, taId);
                    if(taFmId) taProcessedFmIds[taId] = taFmId;

                    if (!historicalChain.find(e => e.id === taId)) { // Ensure TA is at the start if it's the root
                        historicalChain.unshift({
                            id: taId, name: taName, approximateEra: "Ancient", verificationScore: 0, gender: 'male', parentId: undefined, familyUnits:[], clanName: "Tribal Ancestry"
                        } as FullClanElderType);
                    }
                    currentElderToTrace = undefined;
                } else {
                    currentElderToTrace = getFullClanElder(currentElderToTrace.parentId, lineageElderTribeName); // Search within the same tribe hint
                }
            } else {
                currentElderToTrace = undefined;
            }
        ...
        // Determine the starting generation for the historical chain
        // The "lowest" historical elder (primaryLineageElder) will be parent of attachmentPointFmId
        baseGenForHistoricalChain = attachmentPointGen - historicalChain.filter(e => !e.id.startsWith("TA_")).length;
        if (historicalChain.some(e => e.id.startsWith("TA_"))) {
            baseGenForHistoricalChain--; // Account for TA generation
        }
        ...
        for (const elder of historicalChain) {
            ...
            if (isTA) {
                currentGen = baseGenForHistoricalChain; // TA is the oldest
            } else {
                // Find its position relative to TA or start of non-TA chain
                let depth = 0;
                let tempEld: FullClanElderType | undefined = elder;
                while(tempEld && tempEld.parentId && !tempEld.parentId.startsWith("TA_")) {
                    depth++;
                    tempEld = historicalChain.find(e => e.id === tempEld!.parentId); // Search within the constructed chain
                }
            ...
            const fmId = addPerson(
                ...
                currentGen, 
                true, 
                isTA ? undefined : lineageSide, // TA has no side, clan ancestors inherit side
                elder.id // Use original ID
            );

            if (fmId) {
                const memberInArray = members.find(m => m.id === fmId);
                if (memberInArray) {
                    if (previousHistoricalFmIdInChain && memberInArray.id !== previousHistoricalFmIdInChain && !isTA) { // TA has no parent in this model
                        memberInArray.parentId = previousHistoricalFmIdInChain;
                    } else if (elder.parentId && elder.parentId.startsWith("TA_")) {
                         memberInArray.parentId = elder.parentId; // Link to TA's original ID
                    }
                }
                previousHistoricalFmIdInChain = fmId;
            } else {
                // If elder already exists (e.g. TA added by other branch), get its ID
        ...
        if (previousHistoricalFmIdInChain && attachmentPointFmId) {
            ...
            const historicalMemberLinked = members.find(m => m.id === previousHistoricalFmIdInChain); // This should be the FM for primaryLineageElder
            if (attachmentMember && historicalMemberLinked && attachmentMember.id !== previousHistoricalFmIdInChain) {
                attachmentMember.parentId = previousHistoricalFmIdInChain;
                console.log(`[transform] Linked ${attachmentMember.name} (gen ${attachmentMember.generation}) to historical elder ${historicalMemberLinked.name} (ID: ${previousHistoricalFmIdInChain}, gen ${historicalMemberLinked.generation})`);
            }
        }
    };
    ...
    if (extendedFamily.maternalLineageElderRef?.id && highestMaternalAncestorFmId) {
        const matAttachId = (idMap[motherRKey] && highestMaternalAncestorFmId === idMap[motherRKey])
            ? highestMaternalAncestorFmId
            : (idMap[mgfRKey] && highestMaternalAncestorFmId === idMap[mgfRKey])
                ? highestMaternalAncestorFmId
                : (idMap[motherRKey] || mainPersonGeneratedId) ;
        const matAttachGen = (idMap[motherRKey] && highestMaternalAncestorFmId === idMap[motherRKey])
            ? highestMaternalAncestorGen
            : (idMap[mgfRKey] && highestMaternalAncestorFmId === idMap[mgfRKey])
                ? highestMaternalAncestorGen
                : (idMap[motherRKey] ? -1 : 0);
        addHistoricalLineageBranch(
        ...
        try {
          ...
          if (members && members.length > 0) {
            const membersToInsert = members.map(member => ({
              ...
              spouse_id: member.spouseId || null,     // UNCOMMENTED - Ensure 'spouse_id' column exists in DB
              is_elder: member.isElder,
              gender: member.gender || null,
              side: member.side || null,
              status: member.status || (member.deathYear ? 'deceased' : 'living'),
              notes: member.notes || null,          // UNCOMMENTED - Ensure 'notes' column exists in DB
              photo_url: member.photoUrl || null,   // UNCOMMENTED - Ensure 'photo_url' column exists in DB
              family_tree_id: savedTreeData.id,
            }));

             // **** IMPORTANT DATABASE NOTE FOR YOU ****
            // The 'spouse_id', 'notes', and 'photo_url' fields above are now UNCOMMENTED.
            // Please ensure your 'family_members' table in Supabase
            // has these columns with appropriate types:
            // - spouse_id: UUID (nullable)
            // - notes: TEXT (nullable)
            // - photo_url: TEXT (nullable)
            // If these columns don't exist, the save operation will fail.
